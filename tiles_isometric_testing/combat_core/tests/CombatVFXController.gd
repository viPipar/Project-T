...
func _play_skill_cast_vfx(caster: Node, ability_tag: String, ability_type: int, element_tag: String) -> void:
	...
	if ability_type == 1:
		...
		mc.setup(element_tag, caster_pos, 0.8 * sm)
		add_child(mc)
	elif ability_type == 2:
		shake_power = 3.0
		var vfx = Sprite2D.new()
		vfx.set_script(preload("res://combat_core/tests/SkillCastVFX.gd"))
		vfx.setup("res://assets/brackeys_vfx_bundle/predrawn/charge_7x6.png", 7, 6, 24.0, 0.9 * sm)
		...
		match tag_lower:
			"epimorphic", "autotomy":
				var vfx = Sprite2D.new()
				vfx.set_script(preload("res://combat_core/tests/SkillCastVFX.gd"))
				vfx.setup("res://assets/brackeys_vfx_bundle/predrawn/vortex_6x5.png", 6, 5, 24.0, 1.0 * sm)
				vfx.global_position = caster_pos
				add_child(vfx)
				shake_power = 4.0
			"divine_departure", "grand_escape":
				var vfx = Sprite2D.new()
				vfx.set_script(preload("res://combat_core/tests/SkillCastVFX.gd"))
				vfx.setup("res://assets/brackeys_vfx_bundle/predrawn/lightstreaks_6x5.png", 6, 5, 24.0, 1.0 * sm)
				vfx.global_position = caster_pos
				add_child(vfx)
				shake_power = 5.0
			"boss_cleave", "boss_great_bash":
				var vfx = Sprite2D.new()
				vfx.set_script(preload("res://combat_core/tests/SkillCastVFX.gd"))
				vfx.setup("res://assets/brackeys_vfx_bundle/predrawn/explosion_6x5.png", 6, 5, 24.0, 1.5 * sm)
				vfx.global_position = caster_pos
				add_child(vfx)
				shake_power = 14.0
			_:
				var vfx = Sprite2D.new()
				vfx.set_script(preload("res://combat_core/tests/SkillCastVFX.gd"))
				vfx.setup("res://assets/brackeys_vfx_bundle/predrawn/big_hit_6x5.png", 6, 5, 24.0, 1.0 * sm)
				vfx.global_position = caster_pos
				add_child(vfx)
