...
func _handle_input(player_id: int, window: InspectWindow, cursor: Node2D, side: Control) -> void:
	...
	# If we just deactivated relic focus, consume the keypress and do nothing
	var meta_key = "relic_focus_just_deactivated_p%d" % player_id
	if InputManager != null and InputManager.has_meta(meta_key) and InputManager.get_meta(meta_key):
		InputManager.set_meta(meta_key, false)
		return
